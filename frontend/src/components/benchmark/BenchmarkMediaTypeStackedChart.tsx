...
import {
  buildMediaTypeStackedPanels,
  type MediaTypePlatformPanel,
} from '../../lib/benchmark/mediaTypeChart';
import {
...
function computeMediaTypeLayout(
  panels: MediaTypePlatformPanel[]
): { plotInnerH: number; svgHeight: number; legendHeight: number } {
  if (panels.length === 0) {
    return {
      plotInnerH: MIN_PLOT_INNER_H,
      svgHeight: MIN_PLOT_INNER_H + MARGIN.top + MARGIN.bottom,
      legendHeight: 40,
    };
  }
  const maxQuarters = Math.max(...panels.map((p) => p.quarters.length), 0);
  const maxMedia = Math.max(...panels.map((p) => p.mediaTypes.length), 0);
  const plotInnerH =
    MIN_PLOT_INNER_H + Math.min(Math.max(maxQuarters - 4, 0) * 6, 56) + 20;
  const legendRows = Math.max(1, Math.ceil(maxMedia / 2));
...
function layoutStackedBars(
  ...
  panel.quarters.forEach((bar) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      rects.push({
        key: `${panel.platform}-${bar.quarter}-${seg.mediaType}`,
        quarter: bar.quarter,
        quarterLabel: qLabel,
...
const PlatformStackedPanel: React.FC<{
  ...
  const [highlightedMediaType, setHighlightedMediaType] = useState<string | null>(null);
  ...
  const handleLegendMouseMove = useCallback((e: React.MouseEvent<HTMLUListElement>) => {
    ...
    root.querySelectorAll<HTMLElement>('[data-legend-item]').forEach((node) => {
      ...
      if (x >= r.left && x <= r.right && y >= r.top && y <= r.bottom) {
        const m = node.dataset.mediaType;
        if (m) hit = m;
      }
    });
    setHighlightedMediaType((prev) => (prev === hit ? prev : hit));
  }, []);
      ...
      <div ref={wrapRef} className="relative px-2 pb-2 pt-1">
            ...
            <ul className="mt-1.5 space-y-1">
              <li className="flex items-center justify-between gap-3">
                <span className="flex min-w-0 items-center gap-1.5 text-slate-600">
                  ...
                  <span className="truncate">{segmentTooltip.mediaType}</span>
                </span>
        ...
        <div ref={chartMotionRef} className="overflow-visible">
          <svg
            ...
            role="img"
            aria-label={`${panel.platform} media type share stacked bars`}
              ...
              <g className="pointer-events-none">
                {rectsWithPath.map((r) => {
                  ...
                  const dimmed = plotHover
                    ? r.key !== plotHoverKey
                    : highlightedMediaType != null && r.mediaType !== highlightedMediaType;
        ...
        >
          {panel.mediaTypes.map((mediaType) => {
            const active = highlightedMediaType === mediaType;
            const dim = highlightedMediaType != null && !active;
            return (
              <li key={mediaType} className="shrink-0">
                <button
                  type="button"
                  data-legend-item
                  data-media-type={mediaType}
                  aria-pressed={active}
                  onFocus={() => setHighlightedMediaType(mediaType)}
                  onBlur={(ev) => {
                    if (!legendRef.current?.contains(ev.relatedTarget as Node)) {
                      setHighlightedMediaType(null);
                    }
                  }}
                  className={`flex max-w-[11rem] items-center gap-1 rounded-md px-1 py-2 text-left text-[10px] outline-none transition-colors ${
                    dim ? 'opacity-40' : 'opacity-100'
                  } ${active ? 'bg-slate-50' : 'hover:bg-slate-50'} focus-visible:ring-2 focus-visible:ring-sky-300`}
                >
                  <Swatch color={panel.mediaTypeLegendColors[mediaType] ?? '#94a3b8'} />
...
export type BenchmarkMediaTypeStackedChartProps = {
  rows: SectionDatum[];
  metricName: string;
  display: MetricDisplaySpec;
  sectionLabel?: string;
...
export const BenchmarkMediaTypeStackedChart: React.FC<BenchmarkMediaTypeStackedChartProps> = ({
  rows,
  metricName,
  display,
  sectionLabel,
  ...
  const panels = useMemo(() => buildMediaTypeStackedPanels(rows, display), [rows, display]);
  ...
  const layout = useMemo(() => computeMediaTypeLayout(panels), [panels]);

  const title = sectionLabel
    ? `Split by media type · ${sectionLabel}`
    : 'Split by media type';
  ...
  if (panels.length === 0) {
    return (
      ...
      >
        No iOS / Android rows with media-type breakdown for the current filters.
      </section>
    );
  }
  ...
  return (
    <section
      ...
      aria-label="Media type stacked chart"
    >
      <div className="flex items-center justify-between gap-2 border-b border-slate-100 px-4 py-3">
        <div className="flex min-w-0 items-center gap-1.5">
          ...
          <FieldHint
            spec={{
              title: 'Split by media type',
              body: '100% stacked bars by quarter (iOS / Android). Hover segments or legend; metrics such as IAA revenue are % shares',
            }}
            side="top"
            className="shrink-0"
          />
